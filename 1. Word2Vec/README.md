...
**第一个问题：** `torch.mm` 使得 one-hot 行矩阵 [0 1 0 ... 0] 众多的零都进行了计算，但真正有意义的计算只有 one-hot矩阵中为 1 对应的 `W` 那一行原样输出了，若先不考虑batch_size，则 `torch.mm` 进行的运算是：
...
![图4-12](https://github.com/WangYH-BUPT/NLP-Python/blob/main/1.%20Word2Vec/README.md)
